...
class AudioDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.feature_fn_args = (
            kwargs["feature_fn_args"] if "feature_fn_args" in kwargs else {}
        )
        self.label_fn_args = (
            kwargs["label_fn_args"] if "label_fn_args" in kwargs else {}
        )
    ...
    def __getitem__(self, index):
        ...
        if self.does_subsample:
            features_and_labels = self.feature_and_label_fn(
                filepath, self.label_paths[index], self.feature_fn, **self.kwargs
            )
            return features_and_labels
        elif self.include_labels:
            return (
                self.feature_fn(filepath, **self.feature_fn_args),
...
class SwitchBoardLaughterDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.subsample:
            ...
            offset, duration = audio_utils.subsample_time(
                self.df.offset[index],
                self.df.duration[index],
...
class SwitchBoardLaughterInferenceDataset(torch.utils.data.Dataset):
    def __init__(self, audio_path, feature_fn, sr=8000, n_frames=44, channel=1):
        self.audio_path = audio_path
        self.feature_fn = feature_fn
        self.sr = sr
        self.n_frames = n_frames

        self.y, _ = librosa.load(audio_path, sr=sr, mono=False)
        self.y = self.y[channel - 1, :]

        ma = np.abs(np.percentile(self.y, 99.95))
        mi = np.abs(np.percentile(self.y, 0.05))
        ma = max(mi, ma)
        self.y = self.y / ma

        self.y = sigmoid(self.y, -1, 1)
