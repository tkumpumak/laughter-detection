...
def parallel_load_audio_batch(
    files, n_processes, sr=None, offsets=None, durations=None
):
    if offsets is not None and durations is not None:
        return Parallel(n_jobs=n_processes)(
            delayed(librosa_load_without_sr)(
                files[i], sr=sr, offset=offsets[i], duration=durations[i]
            )
...
def pad_sequences(sequences, pad_value=None, max_len=None):
    ...
    sequences = [
        librosa.util.fix_length(np.array(ft), max_len, axis=0, **kwargs)
        for ft in sequences
    ]
    return sequences
...
def featurize_mfcc(
    ...
    **kwargs
    ...
    mfcc_feat = librosa.feature.mfcc(
        y, sr, n_mfcc=13, n_mels=13, hop_length=hop_length, n_fft=int(sr / 40)
    ).T  # [:,1:]
...
def featurize_audio_segments(segments, feature_fn, f=None, y=None, sr=None):
    ...
    for segment in segments:
        feature_list.append(
            feature_fn(f=f, offset=segment[0], duration=segment[1], y=y, sr=sr)
        )
    return feature_list
...
def pad_sequences_with_labels(
    ...
    if (output_vocab is None and one_hot_labels) or (
        input_vocab is None and one_hot_labels
    ):
...
def random_noise(y, sr, noise_signals, min_snr=6, max_snr=30, prob=1.0):
    if np.random.uniform(0, 1) < prob:
        meter = pyln.Meter(sr)
...
def random_augment_strong(y, sr, noise_signals, impulse_responses):
    ...
    functions = shuffle(
        [
            partial(random_stretch, prob=1.0, min_stretch=0.5, max_stretch=1.7),
            partial(random_speed, prob=1.0, min_speed=0.5, max_speed=1.7),
            partial(random_pitch, prob=1.0, min_shift=-18, max_shift=18),
...
def spec_augment(spec, prob=1.0):
...
def subsample_time(
    ...
    start = np.maximum(
        start_time, np.random.uniform(start_time, end_time - subsample_length)
    )
