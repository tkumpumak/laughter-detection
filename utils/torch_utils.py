...
def run_epoch(
    ...
    def _get_ctc_loss(output, trg, device):
        ...
        ctc_input_lengths = torch.full(
            size=(ctc_batch_size,), fill_value=ctc_input_length, dtype=torch.long
        )
        ...
        loss_ctc = nn.CTCLoss()(
            ctc_inputs, ctc_trgs, ctc_input_lengths, ctc_trg_lengths
        )
    ...
    def _eval_batch(
        ...
        with torch.no_grad():
            ...
            input_tensor = (
                torch.from_numpy(np.array(seqs)).float().permute((1, 0, 2)).to(device)
            )
    ...
    if iterator is not None:
        ...
        for i, batch in tqdm(enumerate(iterator)):
            ...
            if (
                ...
                if verbose:
                    ...
                    print(
                        "Train loss: ", train_loss_at_step, " | Train PPL: ", train_ppl
                    )
        ...
        for batch_index in range(len(batches)):
            epoch_loss += _run_batch(
                model, device, batches[batch_index], loss_type, teacher_forcing_ratio
            )
...
class Predictor:
    ...
    def predict(self):
        ...
        for i_batch, batch in enumerate(self.generator):
            ...
            with torch.no_grad():
                src = (
                    torch.from_numpy(np.array(seqs)).float().permute((1, 0, 2)).cpu()
                )  # .to(device)
            ...
            readable_preds = [
                text_utils.readable_outputs(s, self.reverse_vocab) for s in output_seqs
            ]
...
class OneFileDatasetPredictor:
    ...
    def predict(self):
        ...
        for i_batch, batch in enumerate(self.generator):
            ...
            with torch.no_grad():
                src = (
                    torch.from_numpy(np.array(seqs)).float().permute((1, 0, 2))
                )  # .to(device)
